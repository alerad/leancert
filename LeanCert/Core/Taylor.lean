...
-- The coercion ℕ → WithTop ℕ∞ goes through ℕ → ℕ∞ → WithTop ℕ∞
-- Many Nat.cast_* lemmas need AddLeftMono (WithTop ℕ∞) which isn't available,
-- so we go through the two-step coercion explicitly.
