...
| Matrix positive-semidefiniteness and positive-definiteness | `matrixPSDCheck`, `matrixPosDefCheck` | `verify_matrix_posSemidef`, `verify_matrix_posDef` in `Validity/MatrixPositivity.lean` | Exact rational Gram/LDLᵀ certificates conclude properties of the entrywise real cast. Automatic discovery is bounded and untrusted; the retained checker runs once. |
