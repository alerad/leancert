...
| Bernstein polynomial bounds | `Validity.Bernstein.checkPolyLowerBound`, `checkPolyUpperBound`, `checkPolyStrictLowerBound`, `checkPolyStrictUpperBound` | `Validity.Bernstein.verify_poly_*_bound` (and `_Icc` bridges), built on `QPoly.bernsteinCheck_sound` in `Engine/Algebra/QPolyBernstein.lean` | Exact rational Bernstein coefficients with recursive bisection inside one certificate. Applies only to expressions `QPoly.ofExpr` recognises; a polynomial touching the bound at an interior non-dyadic point is reported as exhausted. |
