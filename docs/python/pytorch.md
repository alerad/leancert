...
theorem classifier_robust :
    âˆ€ x âˆˆ inputDomain, âˆ€ Î´ âˆˆ perturbation,
    classifier.forward (x + Î´) = classifier.forward x := by
...
A single transformer layer (LayerNorm â†’ FFN â†’ Residual â†’ LayerNorm):
